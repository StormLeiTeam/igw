...
public class XRecyclerView extends RecyclerView {
    ...
    public void refreshComplete(int count) {
        mRefreshHeader.refreshComplate(count);
        previousTotal = getLayoutManager().getItemCount();
    }

    public void refreshComplete(int count, String type) {

        mRefreshHeader.refreshComplate(count, type);
        previousTotal = getLayoutManager().getItemCount();
    }
...
//    public ImageView getRefreshProgress() {
//        if (mRefreshHeader != null) {
//            return mRefreshHeader.getRefreshView();
//        } else {
//            return null;
//        }
//    }
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && (mLoadingListener != null || loadingActionListener != null) && !isLoadingData && loadingMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 0 && lastVisibleItemPosition >=
                ...
                if (loadingActionListener != null) {
                    loadingActionListener.onLoadMore();
                }
            }
        }
    }
    ...
    public void setNoMoreText(String text) {
        if (mFootViews.size() > 0) {
            LoadingMoreFooter footer = (LoadingMoreFooter) mFootViews.get(0);
            footer.setNomoreText(text);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (onPullRefreshLoading != null) {
                    if (deltaY > 10) {
                        onPullRefreshLoading.onPullRefreshClick(false, ev.getRawY());
                    } else if (deltaY < 10) {
                        onPullRefreshLoading.onPullRefreshClick(true, ev.getRawY());
                    }
                }
                if (isOnTop() && pullRefreshEnabled) {
                    mRefreshHeader.onMove(deltaY / DRAG_RATE);
//                    Log.i(TAG, "XRecyclerView  onTouchEvent: " + isOnTop() + "===" + mLastY);
                    if (loadingActionListener != null) {
                        loadingActionListener.onStartRefresh();
            ...
            default:
                ...
                if (isOnTop() && pullRefreshEnabled) {
                    if (mRefreshHeader.releaseAction()) {
                        ...
                        if (loadingActionListener != null) {
                            loadingActionListener.onRefresh();
                            isnomore = false;
                            previousTotal = 0;
                        }
                    }
    ...
    public void setRefreshState() {
        mRefreshHeader.onMove(61 * dm.density);
        mRefreshHeader.setRefreshState();
        if (mLoadingListener != null) {
            mLoadingListener.onRefresh();
            isnomore = false;
            previousTotal = 0;
...
//        return false;
    ...
    private LoadingActionListener loadingActionListener;

    public void setLoadingActionListener(LoadingActionListener loadingActionListener) {
        this.loadingActionListener = loadingActionListener;
    }
...
//
//        while (!((parent = parent.getParent()) instanceof FrameLayout)) {
//            // 循环查找viewPager
...
//    }
    public OnPullRefreshLoading onPullRefreshLoading;

    public void setOnPullRefreshLoading(OnPullRefreshLoading onPullRefreshLoading) {
        this.onPullRefreshLoading = onPullRefreshLoading;
    }

    public interface OnPullRefreshLoading {

        void onPullRefreshClick(boolean status, float y);
    }
}
